// Code generated by dol build. DO NOT EDIT.
// source: auto.go

package model

import (
	"github.com/2637309949/dolphin/packages/null"
)

// StudentClass defined
type StudentClass struct {
	//
	SCId null.Int `xorm:"int(11) pk notnull autoincr 's_c_id'" json:"s_c_id" xml:"s_c_id"`
	//
	StuId null.Int `xorm:"int(11) 'stu_id'" json:"stu_id" xml:"stu_id"`
	//
	CmId null.Int `xorm:"int(11) 'cm_id'" json:"cm_id" xml:"cm_id"`
	//
	OfId null.Int `xorm:"int(11) 'of_id'" json:"of_id" xml:"of_id"`
	//
	ScId null.Int `xorm:"int(11) 'sc_id'" json:"sc_id" xml:"sc_id"`
	//
	IfCb null.Int `xorm:"int(11) 'if_cb'" json:"if_cb" xml:"if_cb"`
	//
	Creater null.String `xorm:"varchar(36) 'creater'" json:"creater" xml:"creater"`
	//
	CreateDate null.Time `xorm:"datetime 'create_date'" json:"create_date" xml:"create_date"`
	//
	Updater null.String `xorm:"varchar(36) 'updater'" json:"updater" xml:"updater"`
	//
	UpdateDate null.Time `xorm:"datetime 'update_date'" json:"update_date" xml:"update_date"`
	//
	ScState null.Int `xorm:"int(11) 'sc_state'" json:"sc_state" xml:"sc_state"`
	//
	ZcClass null.Int `xorm:"int(11) 'zc_class'" json:"zc_class" xml:"zc_class"`
	//
	ScPrice null.Int `xorm:"int(11) 'sc_price'" json:"sc_price" xml:"sc_price"`
	//
	JoinSource null.Int `xorm:"int(11) 'join_source'" json:"join_source" xml:"join_source"`
	//
	Isdelete null.Int `xorm:"int(11) 'isdelete'" json:"isdelete" xml:"isdelete"`
	//
	OsId null.Int `xorm:"int(11) 'os_id'" json:"os_id" xml:"os_id"`
	//
	OsMoney null.Float `xorm:"float(10,2) 'os_money'" json:"os_money" xml:"os_money"`
	//
	FinalMoney null.Float `xorm:"float(10,2) 'final_money'" json:"final_money" xml:"final_money"`
	//
	OnePrice null.Float `xorm:"float(10,2) 'one_price'" json:"one_price" xml:"one_price"`
	//
	RefundOnePrice null.Float `xorm:"float(10,2) 'refund_one_price'" json:"refund_one_price" xml:"refund_one_price"`
	//
	VisitId null.Int `xorm:"int(11) 'visit_id'" json:"visit_id" xml:"visit_id"`
	//
	BussType null.Int `xorm:"int(11) 'buss_type'" json:"buss_type" xml:"buss_type"`
	//
	OutDate null.Time `xorm:"datetime 'out_date'" json:"out_date" xml:"out_date"`
	//
	Remark null.String `xorm:"varchar(100) 'remark'" json:"remark" xml:"remark"`
}

// TableName table name of defined StudentClass
func (m *StudentClass) TableName() string {
	return "student_class"
}
