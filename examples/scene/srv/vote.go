...
func VoteLike(ctx *gin.Context, db *xorm.Engine, params model.VoteInfo) (interface{}, error) {
	...
	if params.Type.Int64 == 101 {
		// 统计文章点赞数目
		if _, err := app.RedisClient.SAdd(context.Background(), fmt.Sprintf("post_user_like_set_incr_%v", params.PostId.String), params.UserId.String).Result(); err != nil {
			return nil, err
		}
		// 用户查找列表时, 需要判断该文章是否已经点赞
		if _, err := app.RedisClient.SAdd(context.Background(), fmt.Sprintf("user_post_like_set_%v", params.UserId.String), params.PostId.String).Result(); err != nil {
			return nil, err
		}
	} else if params.Type.Int64 == 102 {
		// 统计文章点赞数目
		if _, err := app.RedisClient.SRem(context.Background(), fmt.Sprintf("post_user_like_set_incr_%v", params.PostId.String), params.UserId.String).Result(); err != nil {
			return nil, err
		}
		// 用户查找列表时, 需要判断该文章是否已经点赞
		if _, err := app.RedisClient.SRem(context.Background(), fmt.Sprintf("user_post_like_set_%v", params.UserId.String), params.PostId.String).Result(); err != nil {
			return nil, err
		}
	}
	// 3.写入post_user_like_{$post_id}{$user_id}
	if _, err := app.RedisClient.HMSet(context.Background(), fmt.Sprintf("post_user_like_%v%v", params.PostId.String, params.UserId.String), "create_time", time.Now().Value().Format("2006-01-02 15:04"), "type", params.Type.Int64).Result(); err != nil {
		return nil, err
	}
	// 4.新增计数
	num, err := app.RedisClient.SCard(context.Background(), fmt.Sprintf("post_user_like_set_incr_%v", params.PostId.String)).Result()
	if err != nil {
		logrus.Error(err)
		return nil, err
	}
	if _, err := app.RedisClient.Set(context.Background(), fmt.Sprintf("post%v_counter", num), params.PostId.String, 0).Result(); err != nil {
		return nil, err
	}
	return "ok", nil
}
			...
			if db != nil {
				...
				items := funk.Filter(v, func(v1 model.UserLikePost) bool {
					for i3 := range extList {
						if v1.UserId.String == extList[i3].UserId.String && v1.PostId.String == extList[i3].PostId.String {
							return false
						}
					}
					return true
