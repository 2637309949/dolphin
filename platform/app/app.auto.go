...
func SysUserRoutes(engine *Engine) {
	group := engine.Group(viper.GetString("http.prefix"))
	group.Handle("POST", "/sys/user/add", Auth, Roles("admin"), SysUserInstance.Add)
	group.Handle("DELETE", "/sys/user/del", Auth, Roles("admin"), SysUserInstance.Del)
	group.Handle("PUT", "/sys/user/update", Auth, Roles("admin"), SysUserInstance.Update)
...
func init() {
	// Sync models
	App.Manager.MSet().Add(new(model.SysAppFun))
	App.Manager.MSet().Add(new(model.SysArea))
	App.Manager.MSet().Add(new(model.SysAreaTemplate))
	App.Manager.MSet().Add(new(model.SysAreaTemplateDetail))
	App.Manager.MSet().Add(new(model.SysAttachment))
	App.Manager.MSet().Add(new(model.SysClient), "platform")
	App.Manager.MSet().Add(new(model.SysDataPermission))
	...
	App.Manager.MSet().Add(new(model.SysRole))
	App.Manager.MSet().Add(new(model.SysRoleAppFun))
	App.Manager.MSet().Add(new(model.SysRoleMenu))
	App.Manager.MSet().Add(new(model.SysRolePermission))
	App.Manager.MSet().Add(new(model.SysRoleUser))
	App.Manager.MSet().Add(new(model.SysTableColUser))
	App.Manager.MSet().Add(new(model.SysTag))
	App.Manager.MSet().Add(new(model.SysTagGroup))
	App.Manager.MSet().Add(new(model.SysTracker), "platform")
	App.Manager.MSet().Add(new(model.SysUser), "platform")
	App.Manager.MSet().Add(new(model.SysUserBinding))
	App.Manager.MSet().Add(new(model.SysUserTag))
	App.Manager.MSet().Add(new(model.SysUserTemplate))
	App.Manager.MSet().Add(new(model.SysUserTemplateDetail))
	// Async Ctr
	SysAppFunRoutes(App)
	SysAreaRoutes(App)
