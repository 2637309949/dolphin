...
func (ctr *SysTableColumn) SysTableColumnDel(ctx *Context) {
	...
	db := ctx.MustDB()
	ret, err := db.In("id", payload.ID.Int64).Update(&types.SysTableColumn{
...
func (ctr *SysTableColumn) SysTableColumnBatchDel(ctx *Context) {
	...
	ret, err := db.In("id", ids).Update(&types.SysTableColumn{
		UpdateTime: null.TimeFromNow(),
		Updater:    null.IntFromStr(ctx.MustToken().GetUserID()),
		IsDelete:   null.IntFrom(1),
	})
...
func (ctr *SysTableColumn) SysTableColumnPage(ctx *Context) {
	...
	db := ctx.MustDB()
	ret, err := ctr.Srv.DB.PageSearch(db, "sys_table_column", "page", "sys_table_column", q.Value())
	if err != nil {
		logrus.Error(ctx, err)
		ctx.Fail(err)
		return
