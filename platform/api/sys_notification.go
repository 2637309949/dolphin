...
func (ctr *SysNotification) SysNotificationDel(ctx *Context) {
	...
	db := ctx.MustDB()
	ret, err := db.In("id", payload.ID.Int64).Update(&types.SysNotification{
...
func (ctr *SysNotification) SysNotificationBatchDel(ctx *Context) {
	...
	ret, err := db.In("id", ids).Update(&types.SysNotification{
		UpdateTime: null.TimeFromNow(),
		Updater:    null.IntFromStr(ctx.MustToken().GetUserID()),
		IsDelete:   null.IntFrom(1),
	})
...
func (ctr *SysNotification) SysNotificationPage(ctx *Context) {
	...
	db := ctx.MustDB()
	ret, err := ctr.Srv.DB.PageSearch(db, "sys_notification", "page", "sys_notification", q.Value())
	if err != nil {
		logrus.Error(err)
		ctx.Fail(err)
		return
