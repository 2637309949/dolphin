...
func (ctr *SysSchedule) SysScheduleDel(ctx *Context) {
	...
	ret, err := ctx.DB.In("id", payload.ID.Int64).Update(&types.SysSchedule{
...
func (ctr *SysSchedule) SysScheduleBatchDel(ctx *Context) {
	...
	ret, err := ctx.DB.In("id", ids).Update(&types.SysSchedule{
		UpdateTime: null.TimeFromNow(),
		Updater:    null.IntFromStr(ctx.GetToken().GetUserID()),
		IsDelete:   null.IntFrom(1),
	})
...
func (ctr *SysSchedule) SysSchedulePage(ctx *Context) {
	...
	q.SetTags()
	ret, err := ctr.Srv.DB.PageSearch(ctx.DB, "sys_schedule", "page", "sys_schedule", q.Value())
	if err != nil {
		logrus.Error(err)
		ctx.Fail(err)
		return
