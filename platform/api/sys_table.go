...
func (ctr *SysTable) SysTableDel(ctx *Context) {
	...
	db := ctx.MustDB()
	ret, err := db.In("id", payload.ID.Int64).Update(&types.SysTable{
...
func (ctr *SysTable) SysTableBatchDel(ctx *Context) {
	...
	ret, err := db.In("id", ids).Update(&types.SysTable{
		UpdateTime: null.TimeFromNow(),
		Updater:    null.IntFromStr(ctx.MustToken().GetUserID()),
		IsDelete:   null.IntFrom(1),
	})
...
func (ctr *SysTable) SysTablePage(ctx *Context) {
	...
	if ctr.Srv.Report.Check(ctx.Request()) {
		...
		ret, err := ctr.Srv.Report.PageExport(db, "sys_table", "page", "sys_table", q.Value())
		if err != nil {
			logrus.Error(err)
			ctx.Fail(err)
			return
	...
	ret, err := ctr.Srv.DB.PageSearch(db, "sys_table", "page", "sys_table", q.Value())
	if err != nil {
		logrus.Error(err)
		ctx.Fail(err)
		return
