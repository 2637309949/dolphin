...
func (ctr *SysTagGroup) SysTagGroupDel(ctx *Context) {
	...
	ret, err := ctx.MustDB().In("id", payload.ID.Int64).Update(&types.SysTagGroup{
...
func (ctr *SysTagGroup) SysTagGroupBatchDel(ctx *Context) {
	...
	ret, err := ctx.MustDB().In("id", ids).Update(&types.SysTagGroup{
		UpdateTime: null.TimeFromNow(),
		Updater:    null.IntFromStr(ctx.MustToken().GetUserID()),
		IsDelete:   null.IntFrom(1),
	})
...
func (ctr *SysTagGroup) SysTagGroupPage(ctx *Context) {
	...
	q.SetTags()
	ret, err := ctr.Srv.DB.PageSearch(ctx.MustDB(), "sys_tag_group", "page", "sys_tag_group", q.Value())
	if err != nil {
		logrus.Error(err)
		ctx.Fail(err)
		return
