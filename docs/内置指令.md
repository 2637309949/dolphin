...
```shell
[double@double examples]$ dolphin new hello
[double@double examples]$ tree hello
hello/
├── app.properties
├── Dockerfile
├── go.mod
...
```shell
[double@double hello]$ dolphin build
[double@double examples]$ tree hello
hello
├── api
│   ├── app.auto.go
...
```shell
[double@double hello]$ dolphin more idx_blog
...
clean可以用来删除临时文件
```shell
[double@double hello]$ dolphin clean
```

#### serve
serve可以在线浏览swagger文档
```shell
[double@double hello]$ dolphin serve
```
...
```shell
[double@double hello]$ dolphin reverse
```