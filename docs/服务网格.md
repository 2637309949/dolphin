### 服务网格
> 我们使用istio服务网格来实现我们的为服务治理， 一种跟平台和语言无关的服务治理方案
...
```shell
wget https://github.91chifun.workers.dev/https://github.com//k3s-io/k3s/releases/download/v1.21.2%2Bk3s1/k3s-airgap-images-amd64.tar  
wget https://github.91chifun.workers.dev/https://github.com//k3s-io/k3s/releases/download/v1.21.2%2Bk3s1/k3s  
```
...
curl -sfL https://get.k3s.io | INSTALL_K3S_SKIP_DOWNLOAD=true INSTALL_K3S_EXEC="--disable traefik" K3S_KUBECONFIG_MODE=644 sh -
...
```shell
wget https://github.91chifun.workers.dev/https://github.com//istio/istio/releases/download/1.10.2/istio-1.10.2-linux-amd64.tar.gz
```

2. 安装istio

```shell
tar zxvf istio-1.10.2-linux-amd64.tar.gz && cd istio-1.10.2 && sudo cp bin/istioctl /usr/local/bin && istioctl install --set profile=demo -y
```
...
```shell
sudo docker run --privileged -v /home/double/App/docker/rancher/data:/var/lib/rancher -p 8888:80 -p 8443:443 -d rancher/rancher
```
